...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="admin-shell"><div className="admin-container admin-editor"><Link href="/admin?tab=events" className="admin-back">← 이벤트 관리</Link>
    ...
    <section className="admin-section"><h2>참여 현황</h2><div className="admin-participant-list">{participants.length === 0 && <p className="admin-empty">아직 참여자가 없습니다.</p>}{participants.map((item) => <article key={item.id}><div><strong>{item.phone}</strong><span>{item.completed_at ? "스탬프 완료" : "참여 중"}</span></div>{item.completed_at && !item.reward_redeemed_at && <form action={redeemStampReward}><input type="hidden" name="participant_id" value={item.id} /><input type="hidden" name="program_id" value={id} /><button className="secondary-button">기념품 수령 처리</button></form>}{item.reward_redeemed_at && <span className="admin-done">수령 완료</span>}</article>)}</div></section>
