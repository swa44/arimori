...
export default async function StampVerifyPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { data } = await supabase.from("ARIMORI_stamp_participants").select("display_name, completed_at").eq("public_token", token).maybeSingle();
  if (!data) notFound();
  return <div className="event-page page-pattern page-pattern--event"><section className="event-card-page event-verify">
    <p className="eyebrow">ARIMORI EVENT</p><h1>{data.display_name}님의 참여 QR</h1>
