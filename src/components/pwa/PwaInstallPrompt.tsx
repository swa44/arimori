...
export function PwaInstallPrompt() {
  ...
  useEffect(() => {
    ...
    if (standalone || sessionStorage.getItem("arimori-install-dismissed")) return;
  ...
  return (
    <aside className="install-prompt" aria-label="아리모리 앱 설치 안내">
      <span className="install-prompt__icon">{isIos ? <Share size={18} /> : <Download size={18} />}</span>
      <div>
        <strong>아리모리 앱으로 보기</strong>
        <p>{isIos ? "공유 버튼을 누른 뒤 ‘홈 화면에 추가’를 선택하세요." : "홈 화면에 설치하고 빠르게 만나보세요."}</p>
      </div>
